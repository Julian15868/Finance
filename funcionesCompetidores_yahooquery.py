...
options.add_argument('--disable-infobars') #Desactiva la barra de información de Chrome, lo que puede reducir la carga en la memoria.
options.add_argument('--disable-notifications') # Desactiva las notificaciones de Chrome, lo que puede mejorar el rendimiento y la estabilidad.
options.add_argument('--disable-popup-blocking') #Desactiva el bloqueo de ventanas emergentes, lo que puede reducir la carga en la CPU.
options.add_argument('--disable-background-networking') #Desactiva la actividad de red en segundo plano, lo que puede mejorar el rendimiento.
options.add_argument('--disable-background-timer-throttling') #Desactiva la limitación del temporizador en segundo plano, lo que puede mejorar el rendimiento en algunos casos.
options.add_argument('--disable-backgrounding-occluded-windows') #Desactiva la carga de pestañas ocultas, lo que puede mejorar el rendimiento en sistemas con poca memoria.
options.add_argument('--disable-breakpad') #Desactiva el informe de fallos, lo que puede mejorar el rendimiento en algunos casos.
#Con estos de aca abajo no mejoro nada(tampoco)
options.add_argument('--disable-default-apps') # Desactiva las aplicaciones predeterminadas de Chrome, lo que puede reducir la carga en la memoria.
options.add_argument('--disable-logging') #  Desactiva el registro de mensajes en la consola de Chrome, lo que puede reducir la carga en la memoria y mejorar el rendimiento.
options.add_argument('--disable-web-security') # Desactiva las medidas de seguridad del navegador, lo que puede mejorar el rendimiento en algunos casos.
options.add_argument('--dns-prefetch-disable') # Desactiva la precarga de DNS, lo que puede reducir la carga en la red y mejorar el rendimiento.
options.add_argument('--incognito') # Ejecuta Chrome en modo incógnito, lo que puede mejorar el rendimiento en algunos casos.
options.add_argument('--mute-audio') #  Desactiva el audio en el navegador, lo que puede reducir la carga en la CPU y mejorar el rendimiento.
#Ultimo puesto
...
def buscarPrimeraTandaCompetencia(accion,accionPrincipal):
  try:
    ...
    print("No se pudo con el link")
  #HAY UN PROBLEMA CON GUARDAR QUE DEBO SOLUCIONAR
  guardar = "" #AGREGUE ULTIMO
...
def puntajesStocks(stocksTotales,industriaPrincipal,sectorPrincipal):
  ...
  for i in range(len(stocksTotales)):
    puntaje = 0
    #print(stocksTotales[i])
    accion = yq.Ticker(stocksTotales[i])
    try:
      resumenPerfil = accion.asset_profile[stocksTotales[i]]
      #print(resumenPerfil["sector"])
      #print(resumenPerfil["industry"])
      try:
        ...
        print("No se pudo ver el sector o industria")
        puntaje+=0
    except:
      puntaje+=0
      print("No se pudo ver el perfil")
...
def buscarCompetencia(accion,mercados):
  ...
  for i in range(len(mercados)):
    try:
      ...
      driver.implicitly_wait(1)
      driver.get(link)
      page = BeautifulSoup(driver.page_source,"html.parser") #ACA PUSE PARSEER
      driver.quit()
    except:
      continue
